...
export default function StartupList({ 
  ...
  useEffect(() => {
    if (newEngagementType === '投資検討') {
      setIsNewCollabOpen(false);
      setIsNewInvestmentOpen(true);
    } else if (newEngagementType === '事業連携') {
  ...
  const filteredStartups = startups.filter(startup => {
    ...
    const matchesSearch = 
      ...
      (startup.assignedMember && String(startup.assignedMember).toLowerCase().includes(searchTerm.toLowerCase())) ||
  ...
  const handleSubmit = (e) => {
    ...
    const newStartupObj = {
      ...
      bizDevLogs: newBizDevNotes.trim() ? [{
        ...
        author: newAssignedMember.trim() || currentUser?.displayName || currentUser?.email?.split('@')[0] || '担当者',
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Filters Bar & Quick Filter Badges (スマート・モダン化UI) */}
      <div className="rounded-2xl border border-slate-200/80 dark:border-slate-800 bg-white/90 dark:bg-slate-900/85 p-3.5 shadow-sm backdrop-blur space-y-3">
        
        {/* Row 1: Search Bar + Quick Badges + Filter Toggle */}
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-2.5">
          ...
          <div className="relative flex-1 min-w-[240px]">
            ...
            <input 
              type="text" 
              placeholder="企業名、概要、担当者、協業部署、クローズ理由等で検索..." 
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-10 pr-8 py-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-slate-900 dark:text-slate-100 placeholder-slate-400 transition-all text-xs sm:text-sm"
            />
            {searchTerm && (
              <button 
                type="button" 
          ...
          <div className="flex flex-wrap items-center gap-1.5 shrink-0">
            ...
            <button
              type="button"
              onClick={() => setSelectedInvestmentStatus(prev => prev ? '' : '4 DD中')}
              className={`px-2.5 py-1.5 rounded-xl text-xs font-bold transition-all flex items-center gap-1 min-h-[36px] ${
                selectedInvestmentStatus.includes("DD") || selectedInvestmentStatus.includes("投資実行")
            ...
            {/* Expandable Filter Panel Toggle Button */}
            <button
              type="button"
              onClick={() => setIsFilterPanelOpen(prev => !prev)}
              className={`inline-flex items-center space-x-1.5 px-3 py-1.5 rounded-xl font-bold text-xs transition-all min-h-[36px] border ${
                isFilterPanelOpen || (hasActiveFilters && !searchTerm)
                  ? 'bg-blue-600 text-white border-blue-600 shadow-sm'
                  : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 border-slate-200 dark:border-slate-700 hover:bg-slate-200 dark:hover:bg-slate-750'
              }`}
            >
              <SlidersHorizontal className="h-3.5 w-3.5" />
              <span>詳細フィルター</span>
              {hasActiveFilters && (
                <span className="ml-1 px-1.5 py-0.2 rounded-full text-[10px] font-extrabold bg-white text-blue-600 dark:bg-slate-900 dark:text-blue-300">
                  ON
                </span>
              )}
              <ChevronDown className={`h-3 w-3 transition-transform ${isFilterPanelOpen ? 'rotate-180' : ''}`} />
            </button>
            ...
            {hasActiveFilters && (
              <button
                ...
                className="inline-flex items-center space-x-1 px-2.5 py-1.5 rounded-xl text-xs font-bold text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/50 hover:bg-rose-100 border border-rose-200 dark:border-rose-900/40 transition-all min-h-[36px]"
        ...
        {/* Row 2: 展開式 詳細フィルターパネル */}
        {isFilterPanelOpen && (
          <div className="p-4 rounded-xl bg-slate-50/80 dark:bg-slate-950/50 border border-slate-200/80 dark:border-slate-800 animate-fade-in space-y-3.5">
            <div className="flex items-center justify-between border-b border-slate-200/60 dark:border-slate-800/80 pb-2">
              <span className="text-xs font-bold text-slate-600 dark:text-slate-350 uppercase tracking-wider flex items-center gap-1.5">
              ...
              <button
                type="button"
                onClick={clearAllFilters}
                className="text-[11px] font-bold text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 transition-colors"
              >
                全条件をリセット
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <div className="space-y-1">
                <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400">検討Type</label>
                <select
                  value={selectedEngagementType}
                  onChange={(e) => setSelectedEngagementType(e.target.value)}
                  className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg text-xs font-medium text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-1 focus:ring-blue-500"
                >
              ...
              {/* 2. 優先度 */}
              <div className="space-y-1">
                <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400">優先度 (★1〜5)</label>
                <select
                  value={selectedPriority}
                  onChange={(e) => setSelectedPriority(e.target.value)}
                  className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg text-xs font-medium text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-1 focus:ring-blue-500"
                >
                  <option value="">全 優先度</option>
              ...
              {/* 3. 協業ステータス */}
              <div className="space-y-1">
                <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400">協業ステータス</label>
                <select
              ...
              {/* 4. 投資ステータス */}
              <div className="space-y-1">
                <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400">投資ステータス</label>
                <select
              ...
              {/* 5. セクター */}
              <div className="space-y-1">
                <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400">セクター</label>
                <select
              ...
              {/* 6. 調達ステージ */}
              <div className="space-y-1">
                <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400">調達ステージ</label>
                <select
                  value={selectedStage}
              ...
              {/* 7. 復活可能性 */}
              <div className="space-y-1">
                <label className="text-[11px] font-bold text-slate-500 dark:text-slate-400">復活可能性</label>
                <select
                  value={selectedRevivalFeasibility}
                  onChange={(e) => setSelectedRevivalFeasibility(e.target.value)}
                  className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg text-xs font-medium text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-1 focus:ring-blue-500"
                >
                  <option value="">全 復活可能性</option>
              ...
              <div className="space-y-1 flex flex-col justify-end">
                <label className="flex items-center space-x-2 p-2 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 cursor-pointer text-xs font-medium text-slate-700 dark:text-slate-300">
                  <input
                    type="checkbox"
                    checked={hasIncompleteTasksOnly}
                    onChange={() => setHasIncompleteTasksOnly(prev => !prev)}
                    className="rounded border-slate-300 dark:border-slate-700 text-blue-600 focus:ring-blue-500 h-4 w-4"
                  />
                  <span>未完了タスクありのみ</span>
                </label>
              </div>
            </div>
          </div>
        ...
        {/* Filter Hit Count & Status Bar */}
        <div className="flex items-center justify-between pt-1 text-xs text-slate-500 dark:text-slate-400">
          <div>
            該当件数: <strong className="text-slate-800 dark:text-slate-200 font-bold">{filteredStartups.length}</strong> 件
            {hasActiveFilters && (
              <span className="ml-2 text-[11px] text-blue-600 dark:text-blue-400 font-medium">
                (絞り込み適用中)
              </span>
            )}
          </div>
                    ...
                    <div className="space-y-1.5 mb-4 text-xs text-slate-500 dark:text-slate-400">
                      ...
                      {(startup.assignedMember || startup.pic) && (
                        <div className="flex items-center text-slate-700 dark:text-slate-300 font-medium truncate pt-1">
                          <span className="text-blue-500 mr-1.5 shrink-0 text-xs">👤</span>
                          <span className="truncate">担当: {startup.assignedMember || startup.pic}</span>
                        </div>
                      )}
                      {startup.contactPerson && (
                        <div className="flex items-center text-slate-700 dark:text-slate-300 font-medium truncate pt-0.5">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="w-full max-w-2xl bg-white dark:bg-slate-900 rounded-3xl shadow-xl overflow-hidden border border-slate-200 dark:border-slate-800 max-h-[90vh] flex flex-col animate-scale-up">
            ...
            <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-6 flex-1">
              ...
              <div className="p-4 bg-teal-50/50 dark:bg-teal-950/20 border border-teal-100 dark:border-teal-900/30 rounded-2xl space-y-4 transition-all">
                <button
                  type="button"
                  onClick={() => setIsNewCollabOpen(prev => !prev)}
                  className="w-full flex items-center justify-between border-b border-teal-200/50 dark:border-teal-900/50 pb-2 cursor-pointer group select-none text-left"
                >
                  <div className="flex items-center space-x-2">
                    ...
                    {newEngagementType === '投資検討' && (
                      <span className="text-[10px] text-slate-400 font-normal">（投資メイン時は任意）</span>
                    )}
                  </div>
                  <div className="flex items-center space-x-1.5">
                    <span className="text-[10px] font-bold text-teal-600 dark:text-teal-400">
                      {isNewCollabOpen ? '閉じる' : '展開する'}
                    </span>
                    <ChevronDown className={`w-4 h-4 text-teal-600 dark:text-teal-400 transition-transform duration-200 ${isNewCollabOpen ? 'rotate-180' : ''}`} />
                  </div>
                </button>

                {isNewCollabOpen && (
                  <div className="space-y-4 animate-fade-in">
                    {/* 事業開発進捗 (初回進捗ログ・1st Log カード) */}
                    <div className="p-3.5 bg-white/90 dark:bg-slate-900/90 border border-teal-200/80 dark:border-teal-900/60 rounded-2xl space-y-2.5 shadow-2xs">
                      <div className="flex items-center justify-between">
                        <label className="text-xs font-bold text-teal-800 dark:text-teal-300 flex items-center gap-1.5">
                          ...
                          <span>初回事業開発進捗ログ (第1回目の進捗記録 / 任意)</span>
                        </label>
                        <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded-full bg-teal-100 dark:bg-teal-950 text-teal-700 dark:text-teal-300 border border-teal-200 dark:border-teal-800">
                          1st Log
                        </span>
                      </div>

                      {/* ログヘッダープレビュー: 日付と記入者 */}
                      <div className="flex items-center gap-3 text-xs bg-teal-50/60 dark:bg-teal-950/40 px-3 py-1.5 rounded-xl border border-teal-100 dark:border-teal-900/40 text-slate-600 dark:text-slate-350">
                        <div className="flex items-center gap-1 font-mono text-[11px]">
                          <Calendar className="w-3.5 h-3.5 text-teal-600 dark:text-teal-400" />
                          <span>{newCreatedAtDate ? newCreatedAtDate.replace(/-/g, '/') : '本日'}</span>
                        </div>
                        <div className="flex items-center gap-1 text-[11px]">
                          <User className="w-3.5 h-3.5 text-teal-600 dark:text-teal-400" />
                          <span>{newAssignedMember.trim() || currentUser?.displayName || currentUser?.email?.split('@')[0] || '担当者'}</span>
                        </div>
                      </div>

                      <textarea 
                        rows="3"
                        placeholder="初回面談の所感や、現在の具体的な協業検討状況・ネクストアクションを入力（空欄でも登録可能）&#10;例: DX推進部とのPoC検討中。2026年Q3開始を目標に協議。" 
                        value={newBizDevNotes}
                        onChange={(e) => setNewBizDevNotes(e.target.value)}
                        className="w-full px-3.5 py-2.5 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-500/20 text-slate-900 dark:text-slate-100 text-sm transition-all resize-none placeholder:text-slate-400"
                      />
              ...
              <div className="p-4 bg-blue-50/50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/30 rounded-2xl space-y-4 transition-all">
                <button
                  type="button"
                  onClick={() => setIsNewInvestmentOpen(prev => !prev)}
                  className="w-full flex items-center justify-between border-b border-blue-200/50 dark:border-blue-900/50 pb-2 cursor-pointer group select-none text-left"
                >
                  <div className="flex items-center space-x-2">
                    ...
                    {newEngagementType === '事業連携' && (
                      <span className="text-[10px] text-slate-400 font-normal">（事業連携メイン時は任意）</span>
                    )}
                  </div>
                  <div className="flex items-center space-x-1.5">
                    <span className="text-[10px] font-bold text-blue-600 dark:text-blue-400">
                      {isNewInvestmentOpen ? '閉じる' : '展開する'}
                    </span>
                    <ChevronDown className={`w-4 h-4 text-blue-600 dark:text-blue-400 transition-transform duration-200 ${isNewInvestmentOpen ? 'rotate-180' : ''}`} />
                  </div>
                </button>

                {isNewInvestmentOpen && (
                  <div className="space-y-4 animate-fade-in">
                    {/* 投資検討進捗 (初回進捗ログ・1st Log カード) */}
                    <div className="p-3.5 bg-white/90 dark:bg-slate-900/90 border border-blue-200/80 dark:border-blue-900/60 rounded-2xl space-y-2.5 shadow-2xs">
                      <div className="flex items-center justify-between">
                        <label className="text-xs font-bold text-blue-800 dark:text-blue-300 flex items-center gap-1.5">
                          <Briefcase className="w-3.5 h-3.5 text-blue-600" />
                          <span>初回投資検討進捗ログ (第1回目の進捗記録 / 任意)</span>
                        </label>
                      ...
                      {/* ログヘッダープレビュー: 日付と記入者 */}
                      <div className="flex items-center gap-3 text-xs bg-blue-50/60 dark:bg-blue-950/40 px-3 py-1.5 rounded-xl border border-blue-100 dark:border-blue-900/40 text-slate-600 dark:text-slate-350">
                        <div className="flex items-center gap-1 font-mono text-[11px]">
                      ...
                      <textarea 
                        rows="3"
                        placeholder="初回投資所見や、事業優位性・DDの着眼点などを入力（空欄でも登録可能）&#10;例: 独自のアルゴリズムに競合優位性あり。シリーズA以降での本格検討を推奨。" 
                        value={newInvestmentMemo}
                        onChange={(e) => setNewInvestmentMemo(e.target.value)}
                        className="w-full px-3.5 py-2.5 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 text-slate-900 dark:text-slate-100 text-sm transition-all resize-none placeholder:text-slate-400"
                      />
                      <p className="text-[11px] text-slate-400 dark:text-slate-500">
                        ※ 入力した内容は名簿一覧の「💳 投資検討進捗」の最新ログとしてカード表示されます。
                      </p>
                    </div>
